...
export default async function DashPage({
  ...
  if (false as boolean) { // dead code — keep types for future use
    ...
    const day: DashDayMobileProps = {
      ...
      adsTotal: todayAdsTotal, adsFb: todayAdsFb, adsTt: todayAdsTt + todayAdsGmv, adsTtBm: todayAdsTt, adsTtGmv: todayAdsGmv,
      adsTtBmRev: (adsToday.ttAds || []).reduce((s: number, a: Record<string, unknown>) => s + Number(a.conversion_value || 0), 0),
      adsTtGmvRev: (adsToday.gmvMax || []).reduce((s: number, a: Record<string, unknown>) => s + Number(a.gross_revenue || 0), 0),
      adsSp: todayAdsSp,
    ...
    return (
      <section className="section" id="dash-day">
        <DashDaySwitch mobileProps={{
          ...
          adsTotal: todayAdsTotal, adsFb: todayAdsFb, adsTt: todayAdsTt + todayAdsGmv, adsTtBm: todayAdsTt, adsTtGmv: todayAdsGmv,
          adsTtBmRev: (adsToday.ttAds || []).reduce((s, a) => s + Number(a.conversion_value || 0), 0),
          adsTtGmvRev: (adsToday.gmvMax || []).reduce((s, a) => s + Number(a.gross_revenue || 0), 0),
          adsSp: todayAdsSp,
