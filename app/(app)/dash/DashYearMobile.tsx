    ...
    <div style={{ background: "#F8FAFC", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      <div style={{ padding: "12px 10px" }}>
        ...
        {p.months.map((m, i) => {
          ...
          return (
            <div key={m.month} style={{ background: "#fff", border: `1px solid ${isExpM ? "#7C3AED" : "#E2E8F0"}`, borderRadius: 10, padding: "8px 10px", marginBottom: 4, opacity: isFuture ? .5 : 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: isFuture ? "default" : "pointer" }} onClick={() => !isFuture && setExpandedMonth(isExpM ? null : m.month)}>
                ...
                <div style={{ fontSize: 10, fontWeight: 700, width: 50, textAlign: "right", color: m.revenue > 0 ? statusColor(pctKH) : "#D1D5DB" }}>{m.revenue > 0 ? formatVNDCompact(m.revenue) : formatVNDCompact(m.target)}</div>
                <div style={{ fontSize: 9, fontWeight: 700, width: 28, textAlign: "right", color: statusColor(pctKH) }}>{pctKH > 0 ? `${pctKH}%` : ""}</div>
                <div style={{ fontSize: 9, width: 32, textAlign: "right", color: adsColor(adsPctM) }}>{m.ads > 0 ? formatVNDCompact(m.ads) : "—"}</div>
                ...
                <div style={{ marginTop: 6, borderTop: "1px solid #F1F5F9", paddingTop: 6 }}>
                  ...
                  {m.ads > 0 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0", borderTop: "1px solid #F8FAFC", marginTop: 2 }}>
                      ...
                      <span style={{ fontSize: 9, color: adsColor(adsPctM), width: 28, textAlign: "right" }}>{adsPctM.toFixed(1)}%</span>
                    </div>
                  )}
                </div>
              )}
