              ...
              <div key={p.id} style={{ border: isMyTask ? "2px solid #7C3AED" : "1px solid #E4E4E7", borderRadius: 10, overflow: "hidden", marginBottom: 8, background: "#fff" }}>
                ...
                <div style={{ padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid #F3F4F6" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#71717A" }}>{p.sku || "—"} · {p.launch_date || "—"}{hDeadline ? ` → ${hDeadline}` : ""}{handoverUser ? ` · Giao: ${handoverUser.name || handoverToEmail}` : ""}</div>
                  </div>
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#F5F5F7", width: "100%", maxWidth: 680, borderRadius: 16, boxShadow: "0 25px 50px rgba(0,0,0,.25)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          ...
          {(m.end_date || m.handover_deadline || m.phase4?.deadline) && (() => {
            ...
            return (
              <div style={{ background: dl < today ? "#FEF2F2" : daysLeft <= 7 ? "#FFFBEB" : "#F0FDF4", border: `1px solid ${dlClr}33`, borderRadius: 8, padding: "8px 12px", marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ fontSize: 11, color: dlClr, fontWeight: 600 }}>
                  Deadline triển khai: <strong>{dl}</strong>
                </div>
                <span style={{ fontSize: 10, fontWeight: 700, color: dlClr }}>
                  {dl < today ? `Quá hạn ${Math.abs(daysLeft)} ngày` : daysLeft === 0 ? "Hôm nay" : `Còn ${daysLeft} ngày`}
                </span>
              </div>
            );
          })()}
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#F5F5F7", width: "100%", maxWidth: 760, borderRadius: 16, boxShadow: "0 25px 50px rgba(0,0,0,.25)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "#71717A", textTransform: "uppercase", marginBottom: 4 }}>KHO TÀI LIỆU CHUNG</div>
            <input value={driveLink} onChange={(e) => setDriveLink(e.target.value)} placeholder="https://drive.google.com/..." style={{ width: "100%", padding: "6px 10px", border: "1px solid #E4E4E7", borderRadius: 6, fontSize: 12 }} />
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, color: "#16A34A" }}>{checklist.filter((c) => c.done).length}/{checklist.length}</span>
          </div>

          <div style={{ height: 1, background: "#E4E4E7", marginBottom: 4 }} />

          {checklist.map((item, idx) => {
            const asgns = item.assignees || [];
          ...
          {/* Thêm mục mới */}
          <div style={{ display: "flex", gap: 6, padding: "6px 8px", marginTop: 4, alignItems: "center" }}>
            <input type="text" placeholder="Tên mục mới (VD: Video unbox...)" value={newItemName} onChange={(e) => setNewItemName(e.target.value)} style={{ flex: 1, padding: "5px 8px", border: "1px solid #E4E4E7", borderRadius: 4, fontSize: 11 }} />
            <input type="text" placeholder="SL" value={newItemQty} onChange={(e) => setNewItemQty(e.target.value)} style={{ width: 40, padding: "5px 4px", border: "1px solid #E4E4E7", borderRadius: 4, fontSize: 11, textAlign: "center" }} />
            <button type="button" onClick={() => { if (!newItemName.trim()) return; setChecklist([...checklist, { name: newItemName, qty: Number(newItemQty) || 1, assignees: [], deadline: "", done: false, results: [] }]); setNewItemName(""); setNewItemQty(""); }} style={{ padding: "4px 10px", borderRadius: 4, fontSize: 10, fontWeight: 700, background: "#7C3AED", color: "#fff", border: "none", cursor: "pointer" }}>+ Thêm</button>
          </div>
